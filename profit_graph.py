...
if __name__ == '__main__':
    ...
    transactions['reconciled_amount_floating_point_after_split_negative'] = transactions.apply(
        lambda row: (row['reconciled_amount'] - row['split_cost']) / -100.0,
        axis=1
    )
    # transactions = transactions.loc[transactions['account_owner'] == 'Melanie']

    fig = px.pie(transactions, values='reconciled_amount_floating_point_after_split_negative', names='category', title='By category')
    fig.show()
